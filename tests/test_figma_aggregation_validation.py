"""Sixth paired sample: preserve the failed first run and validate repairs."""
...
FIXTURE = Path(__file__).parent / "fixtures" / "figma_aggregation"
SCRIPT = Path(__file__).resolve().parents[1] / "scripts" / "check_export_equivalence.py"
spec = importlib.util.spec_from_file_location("aggregation_export_equivalence", SCRIPT)
...
def test_sixth_version_and_first_run_failure_are_preserved():
    metadata = json.loads((FIXTURE / "metadata.json").read_text())
    assert metadata["version"]["id"] == "d4e341f9-71f7-4fa7-82b8-e9207f842741"
...
def test_all_original_layers_text_and_image_sources_are_preserved(paired):
    ...
    assert len(refs) == len(nodes) == len(actual) == 1136
    assert actual.keys() == refs.keys()
    kinds = Counter(n["type"] for n in walk(paired["actual"]))
    assert kinds["lanhutext"] == 458 and kinds["lanhuimage"] == 214
...
def test_sixth_design_passes_export_equivalence_v2(paired, tmp_path):
    ...
    assert report["passed"], f"Sixth design differs; report: {tmp_path / 'equivalence.json'}"
    assert report["tree"]["actual_nodes"] == report["tree"]["reference_nodes"] == 1250
