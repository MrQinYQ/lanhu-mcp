"""Fourth paired sample: preserve the failed first run and validate repairs."""
...
FIXTURE = Path(__file__).parent / "fixtures" / "figma_network"
SCRIPT = Path(__file__).resolve().parents[1] / "scripts" / "check_export_equivalence.py"
spec = importlib.util.spec_from_file_location("network_export_equivalence", SCRIPT)
...
def test_fourth_version_and_first_run_failure_are_preserved():
    metadata = json.loads((FIXTURE / "metadata.json").read_text())
    assert metadata["version"]["id"] == "83600518-4304-44ba-9c39-cdf6079c6ab0"
...
def test_all_original_layers_text_and_image_sources_are_preserved(paired):
    ...
    assert len(refs) == len(nodes) == len(actual) == 512
    assert actual.keys() == refs.keys()
    kinds = Counter(n["type"] for n in walk(paired["actual"]))
    assert kinds["lanhutext"] == 192 and kinds["lanhuimage"] == 121
...
def test_fourth_design_passes_export_equivalence_v2(paired, tmp_path):
    ...
    assert report["passed"], f"Fourth design differs; report: {tmp_path / 'equivalence.json'}"
    assert report["tree"]["actual_nodes"] == report["tree"]["reference_nodes"] == 563
